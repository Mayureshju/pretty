    ...
    <>
        ...
        <div className="max-w-[1320px] mx-auto px-4 py-3">
          <nav className="flex items-center text-xs md:text-sm text-[#888] flex-wrap gap-1">
            ...
            {product.categories?.[0]?.parent && (
              <>
                <Link href={`/${product.categories[0].parent.slug}/`} className="text-[#B5748A] hover:underline">
            ...
            {product.categories?.[0] && (
              <>
                <Link
                  ...
                  className="text-[#B5748A] hover:underline"
                >
              ...
              {images.map((img, i) => (
                <button
                  ...
                  className={`shrink-0 w-[60px] h-[72px] md:w-[72px] md:h-[86px] rounded-lg overflow-hidden cursor-pointer transition-all duration-200 ${
                    activeImg === i ? "ring-2 ring-[#B5748A] opacity-100" : "opacity-50 hover:opacity-80"
                  }`}
                >
          ...
          <div ref={infoRef} className="lg:w-[45%]">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="shrink-0 w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center hover:border-[#B5748A] transition-colors cursor-pointer"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill={wishlisted ? "#B5748A" : "none"} stroke={wishlisted ? "#B5748A" : "#888"} strokeWidth="1.8">
            ...
            {product.metrics.averageRating > 0 && (
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-sm text-[#B5748A]">{product.metrics.ratingCount} Reviews</span>
              </div>
            )}
                  ...
                  {product.variants.map((v, i) => (
                    <button
                      ...
                      className={`flex flex-col rounded-xl border overflow-hidden transition-all duration-200 cursor-pointer w-[110px] ${
                        activeVariant === i ? "border-[#B5748A] shadow-md" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      {v.image && (
                    ...
                    <div key={addon.name} className="shrink-0 w-[110px] rounded-xl border border-gray-200 overflow-hidden">
                      ...
                      <div className="p-2">
                        ...
                        <div className="flex items-center justify-between mt-1">
                          ...
                          <button className="text-[10px] font-bold text-[#B5748A] border border-[#B5748A] rounded px-2 py-0.5 hover:bg-[#B5748A] hover:text-white transition-colors cursor-pointer">
                            ADD
                          </button>
                        </div>
                      </div>
            ...
            <div className="mt-5 p-4 rounded-xl border border-gray-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <button className="text-xs font-medium text-[#B5748A] flex items-center gap-1 cursor-pointer">
            ...
            <button
              ...
              className="mt-4 w-full flex items-center justify-between p-4 rounded-xl border border-gray-200 hover:border-[#B5748A]/30 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-2">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#B5748A" strokeWidth="2" strokeLinecap="round"><path d="M21.41 11.58l-9-9A2 2 0 0011 2H4a2 2 0 00-2 2v7c0 .53.21 1.04.59 1.41l9 9a2 2 0 002.82 0l7-7a2 2 0 000-2.83z" /><circle cx="7.5" cy="7.5" r="1.5" fill="#B5748A" /></svg>
                <span className="text-sm font-semibold text-[#B5748A]">Offers Available</span>
              </div>
            ...
            {showOffers && (
              <div className="border border-t-0 border-gray-200 rounded-b-xl p-4 -mt-1 text-sm text-[#464646] space-y-2">
                <p className="flex items-center gap-2"><span className="text-[#4CAF50] font-bold">10%</span> off on orders above &#8377;999 with code <span className="font-semibold text-[#B5748A]">BLOOM10</span></p>
          ...
          {trustBadges.map((badge) => (
            <div key={badge.label} className="flex flex-col items-center text-center">
              <div className="w-12 h-12 rounded-full bg-[#f5f7fa] flex items-center justify-center mb-2">
                {badge.icon === "smile" && (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#B5748A" strokeWidth="1.5"><circle cx="12" cy="12" r="10" /><path d="M8 14s1.5 2 4 2 4-2 4-2" /><circle cx="9" cy="9" r="0.5" fill="#B5748A" /><circle cx="15" cy="9" r="0.5" fill="#B5748A" /></svg>
                )}
                {badge.icon === "pin" && (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#B5748A" strokeWidth="1.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" /></svg>
                )}
                {badge.icon === "truck" && (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#B5748A" strokeWidth="1.5"><rect x="1" y="3" width="15" height="13" rx="1" /><path d="M16 8h4l3 3v5h-7V8z" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></svg>
                )}
              </div>
                      ...
                      {p.metrics.averageRating > 0 && (
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          {p.metrics.ratingCount > 0 && (
                            <span className="text-[10px] text-[#B5748A]">({p.metrics.ratingCount.toLocaleString()})</span>
                          )}
                        </div>
                      )}
                    </div>
      ...
      <div className="sticky bottom-0 z-40 bg-white border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="max-w-[1320px] mx-auto px-4 py-3 flex items-center gap-3">
          <button className="flex-1 py-3 rounded-lg border-2 border-[#B5748A] text-[#B5748A] text-sm md:text-base font-semibold transition-colors hover:bg-[#B5748A]/5 cursor-pointer">
            ADD TO CART
          </button>
          <button className="flex-1 py-3 rounded-lg bg-[#B5748A] text-white text-sm md:text-base font-semibold transition-colors hover:bg-[#9E6377] cursor-pointer">
